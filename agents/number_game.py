...
class NumberGameAgent(BaseAgent):
    ...
    def run(self, state: GameState) -> GameState:
        ...
        if response not in ["yes", "no", "higher", "lower"]:
            state.system_message = "Invalid input. Please answer with: yes/higher/lower: "
            state.needs_input = True
            return state

        # Process response (unchanged logic, ensure to clear user_input at end)
        ...
        if response == "yes":
            ...
            if response == "higher":
                new_min = state.number_game_state["guess"] + 1
                state.number_game_state["min"] = new_min
            elif response == "lower":
                new_max = state.number_game_state["guess"] - 1
            ...
            if state.number_game_state["min"] == state.number_game_state["max"]:
                ...
                state.current_game = None
                state.number_game_state = {}
            elif state.number_game_state["min"] > state.number_game_state["max"]:
                state.game_result = (
                    "Hmm, I think you might have changed your number!\n"
                    "Returning to main menu..."
                )
                state.current_game = None
                state.number_game_state = {}
